...
class Feuillosaure(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if attack == True and miss == False and Xallie <= (25/100)*screenwidth +30:
            Xallie += 9
            self.rect.center = (Xallie, (60/100)*screenheight)
        elif attack == True and miss == False:
            Xallie = (25/100)*screenwidth
            self.rect.center = (Xallie, (60/100)*screenheight)
...
def Vierestante():
    screen.blit(NbdePV, (int((8.5/100)*screenwidth), int((19/100)*screenheight)))

def Vieentière():
    screen.blit(NbdePVentier, (int((2/100)*screenwidth), int((19/100)*screenheight)))

def VierestanteEnnemy():
    screen.blit(NbdePVEnnemy, (int((95/100)*screenwidth), int((19/100)*screenheight)))

def VieentièreEnnemy():
    screen.blit(NbdePVEntierEnnemi, (int((88.5/100)*screenwidth), int((19/100)*screenheight)))
            ...
            if B2buttonX <= mouse[0] <= B2buttonX+widthB and B2buttonY <= mouse[1] <= B2buttonY+heightB:
                if click[0] == 1:
                    ...
                    if precision > 1:
                        miss = False
                        if barwidth2 > 0:
                            attackValue = random.randint(int((8/100)*barwidthOG), int((12/100)*barwidthOG))
        ...
        if whatattack > 3:
            if precision > 1:
                if int(current_tour) == 20 and barwidth > 0:
                    missennemi = False
                    attackValue = random.randint(int((13/100)*barwidthOG), int((13/100)*barwidthOG))
