...
################################################### la fonction qu'on avait commencée
Direction=0

def collisions(XNat, YNat, vitesse, Direction, MapPart):
    ...
    whatpart = [["arbre1part1", "house1.1part1", "house1.2part1", "arbre2part1", "arbre3part1", "arbre4part1", "arbre5part1", "Centrepokemon", "Hautarbrepart1", "barriere1part1", "barriere2part1", "barriere3part1"], ["Hautarbrepart2", "Droitearbrepart2", "barriere1part2", "barriere2part2", "barriere3part2", "barriere4part2", "arbre1part2", "arbre2part2"], ["house1", "house2", "bushligne", "barriere1","barriere2", "barriere3",  "barriere4", "house3", "3bush1", "3bush2",  "3bush3", "3bush4", "3bush5", "3bush6"], ["house3bis", "house4", "house5", "house5bis", "marketbis"], ["arbre1part5", "arbre2part5", "GrandArbreHautpart5", "GrandArbreTroncpart5", "arbre3part5", "arbre4part5"], ["arbre1part6", "arbre2part6", "arbre3part6", "arbre4part6", "arbre5part6"]]

    if Direction == 1: #gauche
        for nom in whatpart[MapPart-1]:
            if MapPart == (1 or 3 or 5):
                if (XNat > vitesse+50):
                    XNat -= vitesse
            else:
                if (not((obstacles[nom][0] < XNat < obstacles[nom][1]+9) and (obstacles[nom][2] < YNat < obstacles[nom][3]))):
                    XNat -= vitesse


    if Direction == 2: #droite
        for nom in whatpart[MapPart-1]:
            if MapPart == (2 or 4 or 6):
                if (XNat < screenwidth - vitesse - 50):
                    XNat += vitesse
            else:
                if (not((obstacles[nom][0]-9 < XNat < obstacles[nom][1]) and (obstacles[nom][2] < YNat < obstacles[nom][3]))):
                    XNat += vitesse
    ...
    if Direction == 3: #haut
        for nom in whatpart[MapPart-1]:
            if MapPart == (1 or 2):
                ...
                if (not((obstacles[nom][0] < XNat < obstacles[nom][1]) and (obstacles[nom][2] < YNat < obstacles[nom][3]+9))):
                    YNat -= vitesse
    ...
    if Direction == 4: #bas
        for nom in whatpart[MapPart-1]:
            if MapPart == (5 or 6):
                ...
                if (not((obstacles[nom][0] < XNat < obstacles[nom][1]) and (obstacles[nom][2]-9 < YNat < obstacles[nom][3]))):
                    YNat += vitesse
...
plantes={"part6": [838, 1018, 190, 446]}
...
while running:
    ...
    if txtChallengeKal == False:   
        ...
        if 1025<XNat<1075 and 250<YNat<260 and MapPart3==True and Haut == True:
            MapPart3=False
            LAB=True
        ...
        if 500<XNat<700 and YNat>=screenheight and LAB==True:
            LAB=False
            MapPart3=True
    ...
    if txtChallengeKal == True:   #un combat se lance si on va voir le dresseur ennemi OU si on rentre dans une petite zone d'herbe en partie 6
